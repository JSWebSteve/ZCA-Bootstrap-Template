...
if (PRODUCT_LIST_ALPHA_SORTER === 'true') {
    ...
    if (TEXT_PRODUCTS_LISTING_ALPHA_SORTER !== '') {
        echo '<label class="form-label me-2" for="select-alpha_filter_id">' . TEXT_PRODUCTS_LISTING_ALPHA_SORTER . '</label>';
    } else {
        echo '<label class="form-label visually-hidden" for="select-alpha_filter_id">' . TEXT_PRODUCTS_LISTING_ALPHA_SORTER_NAMES . '</label>';
    }
    echo zen_draw_pull_down_menu('alpha_filter_id', $letters_list, ($_GET['alpha_filter_id'] ?? ''), 'id="select-alpha_filter_id" class="form-select" onchange="this.form.submit()" aria-label="' . TEXT_PRODUCTS_LISTING_ALPHA_SORTER_NAMES . '"');
}
