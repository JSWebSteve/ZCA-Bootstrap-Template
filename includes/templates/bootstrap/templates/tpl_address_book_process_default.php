...
if (isset($_GET['delete'])) {
?>
    <div class="required-info text-end mb-3"><?php echo DELETE_ADDRESS_DESCRIPTION; ?></div>

    <address><?php echo zen_address_label($_SESSION['customer_id'], $_GET['delete'], true, ' ', '<br>'); ?></address>
    <br class="clearBoth">
 
    <div id="addressBookDelete-btn-toolbar" class="d-flex justify-content-between" role="toolbar">
        ...
        <?php echo zen_image_submit(BUTTON_IMAGE_DELETE, BUTTON_DELETE_ALT, '', 'btn btn-danger'); ?>
...
?>
        ...
        <div id="addressBookEdit-btn-toolbar" class="d-flex justify-content-between" role="toolbar">
        <?php echo zca_button_link(zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'), BUTTON_BACK_ALT, 'button_back'); ?>
        <?php echo zen_draw_hidden_field('action', 'update') . zen_draw_hidden_field('edit', (int)$_GET['edit']) . zen_image_submit(BUTTON_IMAGE_UPDATE, BUTTON_UPDATE_ALT, '', 'btn btn-primary'); ?>
    </div>
<?php
    } else {
?>
    <div id="addressBookNew-btn-toolbar" class="d-flex justify-content-between" role="toolbar">
        <?php echo zca_button_link(zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'), BUTTON_BACK_ALT, 'button_back'); ?>
        <?php echo zen_draw_hidden_field('action', 'process') . zen_image_submit(BUTTON_IMAGE_SUBMIT, BUTTON_SUBMIT_ALT, '', 'btn btn-primary'); ?>
    </div>
